...
class MyHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        experts_num = hp.Int('experts_num', min_value=1, max_value=3, step=1)
        ...
        # experts------------------------------------------------------------------------------------------------------
        experts_out = []
        expert_out_units = hp.Int('expert_out_unit', min_value=8, max_value=32, step=8)
        for i in range(experts_num):
            expert_fc_out = inputs
            for j in range(hp.Int('experts%d_fc' % i, min_value=1, max_value=2, step=1)):
                expert_fc_out = tf.keras.layers.Dense(units=hp.Int('units%d' % j, min_value=16, max_value=64, step=16),
        ...
        for i in range(hp.Int('dloc_tower_fc', min_value=1, max_value=2, step=1)):
            dloc_tower_out = tf.keras.layers.Dense(units=hp.Int('units%d' % i, min_value=16, max_value=64, step=16),
        ...
        for i in range(hp.Int('ed_tower_fc', min_value=1, max_value=2, step=1)):
            ed_tower_out = tf.keras.layers.Dense(units=hp.Int('units%d' % i, min_value=16, max_value=64, step=16),
        ...
        for i in range(hp.Int('ed_tower_fc', min_value=1, max_value=2, step=1)):
            overload_tower_out = tf.keras.layers.Dense(units=hp.Int('units%d' % i, min_value=16, max_value=64, step=16),
